...
def move_to_position_via_route_op(state,a):
    state.squad_state = 'move'
    return state

def choose_route_op(state,desiredRoute):
    state.desiredRouteIndex=desiredRoute
    state.squad_state = 'move'
    return state
...
pyhop.declare_operators(choose_position_op,move_to_position_via_route_op,choose_route_op,locate_at_position_op,scan_for_enemy_and_assess_exposure_op,null_op,aim_op,shoot_op)
...
def choose_position_m(state,param,a):
    nextPosition=param
    "constructing routs to next position:"
    origin = state.loc
    destination = ext_funs.getLocation(state, nextPosition)
    min_index = np.argmin(state.distance_from_assesedBlues)
    avoid_location = state.assesedBlues[min_index].location
    navigatedUnitName = state.commanderName
    state.avaliableRouts = ext_funs.getAvaliableRouts(origin, destination, avoid_location, navigatedUnitName)
...
def move_to_position_by_route_m(state,param,a):
    desired_path=param
    return [('choose_route_op',desired_path),('move_to_position_via_route_op',a),('locate_at_position_op',a),('scan_for_enemy',a)]

pyhop.declare_methods('move_to_position', move_to_position_by_route_m)
pyhop.declare_original_methods('move_to_position', move_to_position_by_route_m)
...
def findplan(basicRanges,squadPosture,enemyDimensions,loc,blueList,BluePolygonCentroid,AccuracyConfiguration,bluePolygon,commanderName):
    ...
    init_state.avaliableRouts=None
    init_state.desiredRouteIndex=None
    # Scenario Data:
