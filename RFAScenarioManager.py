...
class RFAScenarioManager:
    def __init__(self):
        ...
        self.config['attack_position_deployment_mean_time'] = float(
            self.configuration.at['attack_position_deployment_mean_time', 'value'])
        self.config['attack_position_deployment_std_time'] = float(
            self.configuration.at['attack_position_deployment_std_time', 'value'])
        self.config['open_position_deployment_mean_time'] = float(
            self.configuration.at['open_position_deployment_mean_time', 'value'])
        self.config['open_position_deployment_std_time'] = float(
            self.configuration.at['open_position_deployment_std_time', 'value'])
        self.config['attack_position_departure_mean_time'] = float(
            self.configuration.at['attack_position_departure_mean_time', 'value'])
        self.config['attack_position_departure_std_time'] = float(
            self.configuration.at['attack_position_departure_std_time', 'value'])
        self.config['open_position_departure_mean_time'] = float(
            self.configuration.at['open_position_departure_mean_time', 'value'])
        self.config['open_position_departure_std_time'] = float(
            self.configuration.at['open_position_departure_std_time', 'value'])
                        ...
                        if Gui_entity.HTNtarget != [] and Gui_entity.HTNtarget != None:
                            if len(Gui_entity.HTNtarget[0]) == 2:
                                if Gui_entity.HTNtarget[0][1] == 'real':
                                    ...
                                    gui_target = str(
                                        str(Gui_entity.HTNtarget[0][0]))  # +" based on intelligence assesment")

                        gui_update_thread = threading.Thread(target=self.gui.updatemed,
                                                             args=[coa_names, str(current_task), geoDest,
                                                                   str(gui_target)])
                ...
                for i in range(len(self.entity_list)):
                    ...
                    if current_entity.role == "co":  # commander roll type
                        ...
                        if self.config['awareness_type'] == 'sa':
                            ...
                            current_entity.enemies_relative_direction = ext_funs.enemies_relative_directionFromFusionreport(
                                current_entity, self.fusionReport)
                        ...
                        if current_entity.state == PositionType.MOVE_TO_OP:
                            if self.config['awareness_type'] == 'direct':
                                losRespose_vec = losOperatorlist(self.squadPosture, self.enemyDimensions,
                                                                 self.blue_entity_list,
                                                                 current_entity.current_location)
                                ext_funs.updateBlueEntitiesAndRelativeDirectionVecFromlosRespose_vec(self,
                                                                                                     current_entity,
                                                                                                     losRespose_vec,
                                                                                                     "squad_eyes_range")
                        ...
                        if current_entity.planBool == 1 and current_entity.COA == []:
                            ...
                            current_entity.plan = []
                            gui_update_thread = threading.Thread(target=self.gui.updatemed, args=[current_entity.plan,
                                                                                                  str(current_entity.current_task)])
                            gui_update_thread.start()
                            plan = htnModel.findplan(self.config,
                                                     ...
                                                     self.AccuracyConfiguration,
                                                     next(x for x in self.Polygons if x['areaName'] == 'BluePolygon')[
                                                         'polygon'],
                            ...
                            for primitive_task in self.entity_list[i].COA:
                                if primitive_task[0] == 'shoot_op':
                                    if (primitive_task[1]) != None:
                                        target = next(
                                            x for x in self.blue_entity_list_HTN if x.unit_name == (primitive_task[1]))
                        ...
                        "next task implementation"
                        if current_entity.alive and (((entity_next_state_and_action.takeAction == 1) and (
                                current_entity.preGameBool == False)) or \
                                                     ((current_entity.preGameBool == True) and (
                                                             entity_next_state_and_action.takeAction == 0))):
                            if current_entity.preGameBool == True:
                                current_entity.preGameBool = False
                            current_entity.current_task = current_entity.COA[0][0]
                            current_entity.COA.pop(0)
    ...
    def entity_next_state_and_action_impleneter(self, entity_next_state_and_action, current_entity):
                ...
                if current_entity.face != "current_position":
                    if self.config['move_type'] == "destination":
                        ...
                        for enemy in self.blue_entity_list_HTN:
                            if (enemy.location['latitude'] != None and
                                ...
                                if enemy.is_alive == True:
                                    ...
                                    enemy_copy.distFromSquad = ext_funs.getMetriDistance(
                                        current_entity.current_location,
                                        enemy.location)
                        ...
                        if enemies_location_list != []:
                            ...
                            enemies_location_list = sorted(enemies_location_list, key=lambda x: (-x.distFromSquad),
                                                           reverse=True)
                            ignore_location = enemies_location_list[0].location
                            logging.debug(
                                "mobility agent try to avoid enemy: " + str(enemies_location_list[0].unit_name))
        ...
        # LOS:
            ...
            logging.debug(
                "Squad is scanning for enemies from Attack position: " + str(current_entity.face) + ". " + str(
                    "scan range is: ") + str(self.basicRanges['squad_view_range']) + " meters")
            ...
            if aim_list != []:
                ...
                for enemy in aim_list:
                    if (enemy.classification == EntityTypeEnum.OHEZ) or \
                            (enemy.classification == EntityTypeEnum.SUICIDE_DRONE) or \
                            (enemy.classification == EntityTypeEnum.UNKNOWN):
                        if enemy.distFromSquad < self.basicRanges['ak47_range'] and current_entity.vulnerability > \
                                self.config['vulnerability_threshold']:
                            aim_list.remove(enemy)
                            aim_list.insert(0, enemy)
                            logging.debug(
                                "Drone type enemy has been pushed to the top of the aim list due to an emergency situation")
        ...
        # Names are specific for anti_tank squad
            ...
            if target.classification == EntityTypeEnum.EITAN:
                shooting_entity = next(x for x in self.entity_list if
                                       x.squad == squad_name and x.role == "co")  # based on uniqeness of at unit in a squad
                ...
                shooting_entities = [x for x in self.entity_list if
                                     x.squad == squad_name and x.classification == EntityTypeEnum.SOLDIER]  # based on uniqeness of at unit in a squad
                if ext_funs.getMetriDistance(current_entity.current_location, target.location) <= self.basicRanges[
                    'ak47_range']:
                    shootCounter = 0
                    amoNumber = 5
                    for entity in shooting_entities:
                        losResponse = ext_funs.losOperator(self.squadPosture, self.enemyDimensions, target,
                        ...
                        if losResponse['los'][0][0] == True:
                            ...
                            self.communicator.FireCommand(str(entity.unit_name), str(target.unit_name), amoNumber,
                                                          "dif")
                        else:
                            logging.debug("entity: " + str(
                                entity.unit_name) + " did not have line of sight to target. Therefore did not shoot")
                    logging.debug(
                        str(int(amoNumber) * int(shootCounter)) + " bullets has been fired at target from " + str(
                            len(shooting_entities)) + " shooting entities")
                else:
        ...
        if current_entity.scanState == isScan.yes:
            ...
            if self.config['awareness_type'] == 'direct':
                losRespose_vec = losOperatorlist(self.squadPosture, self.enemyDimensions, self.blue_entity_list,
                                                 current_entity.current_location)
                ext_funs.updateBlueEntitiesAndRelativeDirectionVecFromlosRespose_vec(self, current_entity,
                                                                                     losRespose_vec, "squad_view_range")
                # updating HTN list which is used when shooting:
                self.blue_entity_list_HTN = ext_funs.getBluesDataFromVRFtoHTN(self.blue_entity_list)
                # Update vulnerability
                current_entity.vulnerability = ext_funs.assess_vulnerability(
                    ...
                    if enemyDistance != None:
                        if (enemy.classification == EntityTypeEnum.OHEZ) or \
                            ...
                            if enemyDistance < self.basicRanges['ak47_range']:
                                logging.debug("Drone type enemy: " + str(
                                    enemy.unit_name) + " has been detected in emergency situation")
                                breakBool = 1
                        "case of fire opportunity lav detection:"
                        if enemy.classification == EntityTypeEnum.EITAN:
                            if enemyDistance < self.basicRanges['javelin_range']:
                                logging.debug(
                                    "very Good shooting Opporunity at Armored vehicle: " + str(enemy.unit_name))
                                breakBool = 1
            # updating HTN list which is used when shooting:
            currTime = time.time()
            if currTime - current_entity.taskTime > self.config['scan_time'] or breakBool == 1:
                ...
                if len(current_entity.scanDetectionList) == 0:
                    logging.debug("No enemy has been detected within " + str(
                        self.config['scan_time']) + " seconds of scanning - Aborting scan")
                else:
                    logging.debug("Aborting scan - due to enemy detection within range")
                    reduced_scanned_names_list = []
                    [reduced_scanned_names_list.append([enemy.unit_name, enemy.is_alive]) for enemy in
                     current_entity.scanDetectionList if
                     [enemy.unit_name, enemy.is_alive] not in reduced_scanned_names_list]
                    print_list = []
