...
class Ext_funs:
    ...
    def simple_update_distance_from_positions(self,state):
        '''
    ...
    def update_distance_from_positions(self,state):
        '''
        function that update Aget distance from each operational position
        '''
        list_of_distances = []
    ...
    # helper functions
    def simple_getLocation(self,state, index):
        return [state.positions[index][0], state.positions[index][1]]

    def getLocation(self,state, index):
        return {'latitude': state.positions[index]['latitude'], 'longitude': state.positions[index]['longitude'],
                'altitude': None, 'id': None}

    def getMetriDistance(self,loc1, loc2):
    ...
    def getBluesDataFromCSV(self,filename):
        with open(filename) as csv_file:
            csv_reader = csv.reader(csv_file, delimiter=',')
            header = next(csv_reader)
            entities = []
    ...
    def getBluesDataFromVRFtoHTN(self,blueList):
        entities = []
    ...
    def losOperator(self, squadPosture, enemyDimensions, enemy, source_location):
        ...
        losRespose = (self.communicator.GetGeoQuery([source_location], [target], True, True))
        return losRespose